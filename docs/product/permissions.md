...
- `/weather`는 승인된 팀원 포털 역할(`member`, `outlet`, `reviewer`, `observer`, `desk`, `team_lead`, `admin`)에 노출/접근을 허용하고 `partner`는 제외한다.
- `app/api/weather/**`는 AuthGate를 통과하지 않으므로 Route Handler에서 서버 세션과 승인된 팀원 포털 profile을 다시 확인한다.
