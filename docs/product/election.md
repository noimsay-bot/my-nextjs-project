...
- 관리자, DESK, 총괄팀장 실제 권한 사용자는 draft/published 상태를 작성, 저장, 게시, 게시종료할 수 있다.
...
- `draft` 또는 `published` 상태의 선거는 partial unique index로 하나만 유지하고, `closed`는 기록으로 보존한다.

## 오버레이
- 홈은 KST 기준 오늘 날짜와 `published` 상태가 일치할 때만 요약 카드를 보여준다.
...
- 게시종료 후 홈, TVU 라이브장비현황, 일정배정 오버레이는 즉시 중단된다.
