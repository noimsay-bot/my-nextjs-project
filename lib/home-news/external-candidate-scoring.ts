...
function hasTrueHeadlineStrength(candidate: ExternalNewsCandidate) {
  ...
  const isNationalHeadline =
    ...
    breakdown.promotionalPenalty <= 34 &&
  ...
  const isStrongProceduralHeadline =
    ...
    breakdown.frontPageLikelihood >= 56 &&
    breakdown.promotionalPenalty <= 28;
...
export function shouldKeepForHomeLivePreview(candidate: ExternalNewsCandidate) {
  ...
  if (breakdown.promotionalPenalty >= 46) return false;
  if (breakdown.localOnlyPenalty >= 40) return false;
...
export function shouldKeepForHomeCategoryFallback(candidate: ExternalNewsCandidate) {
  ...
  if (!breakdown) return false;
  if (!isMajorMediaCandidate(candidate)) return false;
  if (breakdown.promotionalPenalty >= 40) return false;
  if (breakdown.localOnlyPenalty >= 42) return false;
  ...
  switch (candidate.category) {
    case "politics":
      return (
        ...
        breakdown.portalHeadlineLikelihood >= 60 ||
        breakdown.proceduralImportance >= 72 ||
        candidate.tags?.some((tag) => ["지방선거", "후보확정"].includes(tag))
      );
    case "economy":
      return (
        breakdown.nationalImpact >= 74 &&
        (breakdown.frontPageLikelihood >= 50 || breakdown.portalHeadlineLikelihood >= 52)
      );
    case "world":
      return (
        breakdown.nationalImpact >= 74 &&
        (breakdown.frontPageLikelihood >= 48 || breakdown.portalHeadlineLikelihood >= 50)
      );
    case "society":
      return (
        breakdown.portalHeadlineLikelihood >= 56 ||
        breakdown.frontPageLikelihood >= 52 ||
        breakdown.proceduralImportance >= 70 ||
